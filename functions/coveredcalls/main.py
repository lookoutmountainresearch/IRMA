...
def save_local_data(data):
    ...
    if not os.path.exists('data'):
        os.makedirs('data')
    # Save data.
    with open('data/coveredcalls_suggestions.json', 'w') as outfile:
        json.dump(data, outfile, sort_keys=True, indent=4, separators=(',', ': '))
        print("Covered Calls data saved.")
    return


def load_local_data():
    with open('data/coveredcalls_suggestions.json') as json_file:
        data = json.load(json_file)
        print(data)
        print("Covered Calls Suggestions data loaded.")
    return data
...
if __name__ == "__main__":
    ...
    # Save data to Storage Bucket in the cloud
